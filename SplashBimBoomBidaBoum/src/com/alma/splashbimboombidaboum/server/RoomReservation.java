...
public class RoomReservation extends UnicastRemoteObject implements RoomReservationInterface, Address {
	...
	public RoomReservation() throws RemoteException, MalformedURLException, AlreadyBoundException {
		...
		Naming.bind("rmi://" + PREADRRESS + ":" + PORT + "/" + POSTADDRESS, this);
	}
	...
	public synchronized void getRoom(PlayerInterface player) throws RemoteException, MalformedURLException, NotBoundException { // Find an available room (for up to 4 players)
		RoomInterface currentRoom = null;
		...
		for (String room : rooms.keySet()) {
			...
			if (currentRoom.getSize() + currentRoom.getQueue() < currentRoom.getMaxPlayer()) { // We found an existing one
				...
				player.setRoom(currentRoom.roomConnection(player));
				return;
			}
		}

		player.setRoom(createRoom(4).roomConnection(player)); // We join a brand new one

	}

	public synchronized void getRoom(PlayerInterface player, String id) throws RemoteException, MalformedURLException, NotBoundException { // Joins a specific room if exists
		RoomInterface currentRoom = null;
